...
function getWorkloadRating(secondPageText) {
  ...
  for (const match of matches) {
    ...
    if (values[textValue] === undefined) {
      console.error(`Invalid input: unexpected text label "${textValue}"`);
      return null;
    }
...
function getWorkloadRatingNewFormat(secondPageText){
  const values = { "< 2.0": 5, "2.0": 4, "2.5": 3, "3.0": 2, "> 3.0": 1 };
...
// let pdfBufferTestOld = fs.readFileSync("./RSOC_99_Old_Format.pdf");
// let pdfBufferTestNew = fs.readFileSync("./CSEN_79L_New_Format.pdf");

// extractEvalDataFromPdf(pdfBufferTestOld, "RSOC_99_Old_Format.pdf", "2024 Fall");
// extractEvalDataFromPdf(pdfBufferTestNew, "CSEN_79L_New_Format.pdf", "2024 Fall");